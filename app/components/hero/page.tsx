...
const HeroAbout = () => {
  ...
  useEffect(() => {
    ...
    tl.from(heroRef.current, { opacity: 0 })
     .from([textRef.current, imgRef.current], { 
       y: 30, 
       opacity: 0, 
       stagger: 0.3 
     });
    ...
    ScrollTrigger.create({
      ...
      animation: gsap.from(aboutRef.current, {
        y: 50,
        opacity: 0,
        duration: 1
      })
  ...
  return (
    <>
      ...
      <section
        ref={heroRef}
        className="flex flex-col-reverse md:flex-row items-center justify-center min-h-screen px-5 md:px-10 lg:px-20 bg-gradient-to-br from-indigo-900 via-purple-800 to-purple-700 text-white pt-20 md:pt-0"
      >
          ...
          <div className="inline-block bg-black/20 backdrop-blur-sm rounded-lg px-4 py-2 mb-4 md:mb-6">
            <Typewriter
              options={{
                ...
                loop: true,
                cursor: '▌',
                wrapperClassName: 'text-base md:text-lg lg:text-xl font-semibold',
        ...
        <div ref={imgRef} className="md:w-1/2 flex justify-center mb-12 md:mb-0">
          <div className="w-40 h-40 sm:w-48 sm:h-48 md:w-56 md:h-56 lg:w-64 lg:h-64 bg-gradient-to-tr from-cyan-400 to-purple-500 rounded-full p-1 shadow-xl">
          ...
          <div className="flex flex-col md:grid md:grid-cols-2 gap-8 md:gap-12">
            <div className="space-y-4 md:space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                {[
                  ['Age', '16', 'cyan-400'],
                  ['Education', 'Matric', 'purple-400'],
                  ['Quran', 'Hafiz', 'cyan-400'],
                  ['GIAIC', 'Q3', 'purple-400']
                ].map(([title, value, color], index) => (
                  <div key={index} className={`p-3 md:p-4 bg-gray-800 rounded-lg border-l-4 border-${color}`}>
                    <h3 className={`text-${color} text-sm md:text-base font-bold mb-1`}>{title}</h3>
